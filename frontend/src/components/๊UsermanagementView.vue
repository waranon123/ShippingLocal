// frontend/src/views/ManagementView.vue - Updated to show both import options
...
<template>
  <v-container fluid>
          ...
          <v-card-title>
            ...
            <v-menu offset-y>
              <template v-slot:activator="{ props }">
                <v-btn color="success" v-bind="props" prepend-icon="mdi-file-excel" class="mr-2">
                  Import Excel
                  <v-icon right>mdi-chevron-down</v-icon>
                </v-btn>
              </template>
              <v-list>
                <v-list-item @click="showSingleImport = true">
                  <v-list-item-title>
                    <v-icon class="mr-2">mdi-file-table</v-icon>
                    Single Day Import
                  </v-list-item-title>
                  <v-list-item-subtitle>Import individual daily records</v-list-item-subtitle>
                </v-list-item>
                <v-list-item @click="showMonthlyImport = true">
                  <v-list-item-title>
                    <v-icon class="mr-2">mdi-calendar-month</v-icon>
                    Monthly Import
                  </v-list-item-title>
                  <v-list-item-subtitle>Create daily records for entire months</v-list-item-subtitle>
                </v-list-item>
              </v-list>
            </v-menu>
    ...
    <!-- Single Day Import Dialog -->
    <ExcelImport 
      v-if="showSingleImport" 
      v-model:dialog="showSingleImport"
      @imported="handleImported" 
    />

    <!-- Monthly Import Dialog -->
    <MonthlyExcelImport 
      v-if="showMonthlyImport" 
      v-model:dialog="showMonthlyImport"
      @imported="handleImported" 
    />
...
import MonthlyExcelImport from '@/components/ExcelImport.vue' // Will be updated component
...
const showMonthlyImport = ref(false)
...
const downloadTemplate = async () => {
  try {
    ...
    saveAs(blob, 'truck_monthly_import_template.xlsx')
    snackbarStore.success('Monthly template downloaded successfully')
  } catch (error) {
...
const handleImported = (count) => {
  applyFilters()
  selected.value = []
  snackbarStore.success(`${count} records imported successfully`)
}
